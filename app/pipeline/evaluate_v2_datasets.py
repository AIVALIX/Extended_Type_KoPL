...
使用方法:
  python pipeline/evaluate_v2_datasets.py --num-samples 50
  python pipeline/evaluate_v2_datasets.py --dataset one_hop two_hop --num-samples 100
"""
...
DATASETS_V2 = {
    ...
    "two_intersection": {
        ...
        "anchor_keys": ["anchor_a_name", "anchor_b_name"],
    ...
    "three_intersection": {
        ...
        "gold_answers_key": "answer_nodes",
        "is_intersection": True,
        "anchor_keys": ["anchor_a_name", "anchor_b_name", "anchor_c_name"],
...
def check_path_match(
    predicted_path: Optional[List[str]], gold_relations: List[str]
) -> bool:
...
def run_evaluation(
    ...
    transitions_multi: TransitionMapMulti,
    *,
    num_samples: int = 50,
    num_workers: int = 4,
    ...
    def create_pipeline() -> TypeKoPLPipeline:
        return TypeKoPLPipeline(
            transitions=transitions,
            transitions_multi=transitions_multi if is_intersection else None,
        )
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in tqdm(
            as_completed(futures), total=len(futures), desc=f"  {dataset_name}"
        ):
    ...
    metrics = {
        ...
        "path_accuracy": sum(r.path_match for r in results) / total * 100,
...
def main():
    p = argparse.ArgumentParser(description="Evaluate v2 datasets")
    p.add_argument(
        "--dataset",
        type=str,
        nargs="+",
    ...
    for ds_name in datasets_to_eval:
        ...
        if metrics:
            ...
            print(f"    PathAcc: {metrics['path_accuracy']:.1f}%")
    ...
    print("SUMMARY")
    print(f"{'='*60}")
    print(f"{'Dataset':<25} {'Hits@1':>8} {'Hits@10':>8} {'PathAcc':>8} {'MRR':>8}")
    print("-" * 60)
    for ds_name, m in all_metrics.items():
        print(
            f"{ds_name:<25} {m['hits_at_1']:>7.1f}% {m['hits_at_10']:>7.1f}% {m['path_accuracy']:>7.1f}% {m['mrr']:>7.1f}%"
        )
